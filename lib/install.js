...
import { conflictingEntryIds, hasDshManifest, hasLoadableEntry, pluginSubdirs, profileDir, readInstalled, readProfileBundles, repairBrokenFileDeps } from './profile.js';
 ...
 */
export async function withHoistRecovery(run, profile, pluginArgs, explicitDir) {
    ...
    if (!ok(result) && !result.cancelled) {
        ...
        else if (failure?.code === 'broken-file-dep'
